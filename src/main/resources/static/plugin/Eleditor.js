...
(function (w) {
  ...
  const _debug = false;
  const _version = '1.8.9';
  const _namespace = 'Eleditor';
  const _notctname = ['INPUT', 'IMG', 'TEXTAREA'];
  const _toolnames = {
    insertText: '插文字',
    insertImage: '插图片',
    insertLink: '插链接',
  ...
  const _scriptPath = (function (func) {
    let _js = document.scripts;
    _js = _js[_js.length - 1].src.substring(0, _js[_js.length - 1].src.lastIndexOf('/') + 1);
    func(_js);
    return _js;
  }((s) => {
    const _buildLib = `<link rel="stylesheet" href="${s}layout/base.css?v=${_debug ? (+new Date()) : _version}">`;
    $('head').append(_buildLib);
  }));

  const _inArray = function (s, a) {
	    for (const i in a) {
	        if (a[i] == s) {
	            return true;
	        }
  ...
  const _formatInnerText = function (t) {
    let s = t.replace(/\ +/g, '');
    s = s.replace(/[ ]/g, '');
    s = s.replace(/[\r\n]/g, '');
    return s.replace(/(^\s*)|(\s*$)/g, '');
  };
  const _getLayerMaxZIndex = function () {
  ...
  const _genEditorUid = function () {
    return `${_namespace}${+new Date()}`;
  };

  const _correctHtmlStructure = function () {
    const _$wrap = arguments[0];
    const _empty = arguments[1];

    $.each(_$wrap.find('iframe,audio,video'), function () {
      const _$this = $(this);
      const _tagName = this.tagName.toLocaleLowerCase();
  ...
  const _buildEditorModule = function (_toolbars, _uid) {
    const _layerZIndex = _getLayerMaxZIndex();
    let _html = `<div class="Eleditor-wrap" style="z-index:${_layerZIndex}" id="${_uid}">\
						<div class="Eleditor-controller">\
						<ul>`;

    for (let i = 0; i < _toolbars.length; i++) {
      const _it = _toolbars[i];
      const _id = typeof _it === 'object' ? _it.id : _it;
      const _tag = (typeof _it === 'object' && _it.tag) ? _it.tag.toLocaleLowerCase() : null;
      const _name = typeof _it === 'object' ? _it.name : _toolnames[_it];
  ...
  w[_namespace] = function () {
    ...
    /* insert editor */
    let _lastScrollH = null;
    const _$window = $(w);
    const _$scrollWrap = $('html,body');
    const _$editorWrap = $(_buildEditorModule(_args.toolbars, _editorUid));
    const _$editorController = _$editorWrap.find('.Eleditor-controller');
    const _$editorLoadingMask = _$editorWrap.find('.Eleditor-loading');
    const _$editorTextModule = _$editorWrap.find('.Eleditor-textEditor');
    const _$editorTextArea = _$editorTextModule.find('.Eleditor-inputarea .textarea');
    const _$editorUploadImageBtn = _$editorController.find('.Eleditor-insertImage');
    const _$editorBarUploadImageBtn = _$editorTextModule.find('.Eleditor-textStyle-item-upImg');
    const _$editorTextLinkArea = _$editorTextModule.find('.Eleditor-inputarea input');
    const _$editorColorModule = _$editorTextModule.find('.Eleditor-textEditor-colors');
    const _$editorFontsizeModule = _$editorTextModule.find('.Eleditor-textEditor-fontsizes');
    const _$editorUndoBtn = _$editorWrap.find('.Eleditor-undo');
    const _$editorLineheightModule = _$editorTextModule.find('.Eleditor-textEditor-lineheight');
    const _$editorLinedecorationsModule = _$editorTextModule.find('.Eleditor-textEditor-linedecorations');
    const _$editorDeleteLayer = _$editorWrap.find('.Eleditor-delete-layer');
    ...
    /* bindEvent */
    let _$selected = null;
    let _imageUploader = null;

    const _showEditorControllerLayer = function (_$e) {
      _$selected = _$e;
      _$e.addClass('Eleditor-active');

      const _calTop = _$e.offset().top + _$e.outerHeight();

      $.each(_$editorController.find('li'), (i, e) => {
        const _$e = $(e);
        let _tgs = _$e.attr('bind-tags');
        if (_tgs) {
    ...
    const _syncRenderTextEditorView = function () {
      ...
      const _selectAlign = _$selected.css('text-align');
    ...
    if (typeof WebUploader !== 'undefined' && typeof _args.uploader !== 'function') {
      ...
      _imageUploader.on('uploadStart', (_file, _percentage) => {
			    _showLoadingMask('上传图片中<span id="uploadProgress">1</span>%');
      });
      _imageUploader.on('uploadProgress', (_file, _percentage) => {
      ...
      _imageUploader.on('uploadComplete', () => { _hideLoadingMask(); });
      _imageUploader.on('uploadSuccess', (_file, _call) => {
        const _hasStyleBarBtn = $(`#rt_${_file.source.ruid}`).parents('.Eleditor-textStyle-item-upImg').length > 0;

			    if (_call.status == 1) {
          if (_hasStyleBarBtn) {
            _$editorTextArea.append(`<img width="100%" height="auto" src="${_call.url}">`);
          } else {
				    	_appendHistory();

            _$selected.after($(`<img width="100%" height="auto" src="${_call.url}">`));
      ...
      const _uploaderHanlder = function () {
        const _hasStyleBarBtn = $(this).hasClass('Eleditor-textStyle-item-upImg');

        try {
          _args.uploader.call(this).then((_url) => {
            const _urls = typeof _url === 'object' && _url.length ? _url : [_url];

            if (_hasStyleBarBtn) {
              for (var _uindex = 0; _uindex < _urls.length; _uindex++) {
                _$editorTextArea.append(`<img width="100%" height="auto" src="${_urls[_uindex]}">`);
              }
            } else {
					    	_appendHistory();

              for (var _uindex = 0; _uindex < _urls.length; _uindex++) {
                _$selected.after($(`<img width="100%" height="auto" src="${_urls[_uindex]}">`));
              }
    ...
    const _editorModuleEvents = {
      ...
      insertHr() {
        ...
        const _$hr = $('<div class="horizontal-line" style="padding: 10px 0;border-bottom: 1px solid #aaa;margin-bottom: 20px;"></div>');
    ...
    for (let i = 0; i < _args.toolbars.length; i++) {
    ...
    _$editorController.on('click', 'ul li', function () {
      const _$this = $(this);
      const _event = _$this.attr('event');
    ...
    _$editorWrap.on('click', '.Eleditor-delete-revoke', () => {
      _revokeEdit();
    });
    _$editorDeleteLayer.on('click', '.Eleditor-delete-clear', () => {
      if (!confirm('确定清空内容吗？')) {
        return;
      }

      const _$cloneNode = _$wrap.clone();
    ...
    _$editorDeleteLayer.on('click', '.Eleditor-delete-back', () => {
    ...
    _$editorTextModule.on('click', '.Eleditor-textStyle-color,.Eleditor-textStyle-bgColor', function () {
      const _$this = $(this);
      const _role = _$this.hasClass('Eleditor-textStyle-bgColor') ? 'bgcolor' : 'color';
    ...
    _$editorTextModule.on('click', '.Eleditor-textEditor-format', () => {
      const _$cloneTextArea = _$editorTextArea;
      const _removeAttrs = 'style width height border bgcolor align color';

      _$cloneTextArea.removeAttr(_removeAttrs);
      $.each(_$cloneTextArea.find('*'), (_i, _e) => {
        const _$eachElm = _$cloneTextArea.find(_e);
    ...
    _$editorTextModule.on('click', '.Eleditor-textEditor-clean', () => {
    ...
    _$editorTextModule.on('click', '.Eleditor-cancel,.Eleditor-commit', function () {
      ...
      if ($(this).hasClass('Eleditor-commit')) {
        const _style = _$editorTextArea.attr('style') || '';
        const _content = _$editorTextArea.html();
        const _editAct = _$editorTextModule.attr('role') == 'edit';
        const _hasPlaceHolder = _$selected.hasClass('Eleditor-placeholder');
        ...
        if (_editAct || (_hasPlaceHolder && _$editorTextModule.attr('type') != 'link')) {
          ...
          let _buildWordHtml = '';
          if (_$editorTextModule.attr('type') == 'link') {
            const _link = _$editorTextLinkArea.val();
    ...
    _$editorColorModule.on('click', 'ul li span', function () {
      const _color = $(this).css('background-color');
      ...
      } else if (!$(this).hasClass('Eleditor-inheritValue')) {
    ...
    return {
      ...
      append() {
        ...
        const _object = arguments[0];

        if (!_object) {
          return;
        }

        let _$content = $(_object);
